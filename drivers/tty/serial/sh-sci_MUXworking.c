...
// #ifdef USE_MULTIPLEXER
// static int sd_start_rxtx_irq(unsigned char *data, unsigned int len, unsigned char *rx_data)
// {
...
//static inline void sci_start_rx(struct uart_port *port, unsigned int tty_start);

// static inline void scif_start_tx(struct uart_port *port);
// static inline void scif_stop_tx(struct uart_port *port);
// static inline void scif_stop_rx(struct uart_port *port);
// static inline void scif_start_rx(struct uart_port *port, unsigned int tty_start);
// 
...
static int sd_request_irq(void *ptr);
...
#define to_modem_queue(m, c)   do { \
  ...
  else {printk(KERN_ERR "%s no buf space  %d\n", __FUNCTION__, modem_qerr_cnt); modem_qerr_cnt++;}} while(0)
...
#define POLLS_PER_MODEM_SLICE       (6)
...
//        TRACE(0x88);
        ...
        {
//                 TRACE(flow_ctrl +
//                   0x10*(last_tty_x_request==XOFF?1:0) +
//                   0x20*(last_mux_x_request==XOFF?1:0) +
//                   0x40*(last_mod_x_request==XOFF?1:0) );
        }
        sci_start_tx(port);
...
static int sdmodem_muxhandler(void* pv_data)
{
        ...
        //dbg!!!!! modem_connected = PIN_CHK_GSM;
...
//                                 sdIrqData.error = 0;
//                                 sdIrqData.errorCnt = 0;
//                                 sdIrqData.errorPos = 0;

                                //printk(KERN_INFO "~~>> %s - start cmd=%d\n", __FUNCTION__, sd_queue[sd_queue_rx].cmd);
//                                 switch (sd_queue[sd_queue_rx].cmd) {
//                                /* Note: The executing thread is blocked while these handler fcts aren't finished */
//                                 case CMD_SD_WELCOME:
//                                         //TRACE(0x80);
//                                         do_sd_init_hw(0);
//                                         retval = sd_welcome_card_action( sd_queue[sd_queue_rx].host );
//                                         break;
// 
//                                 case CMD_SD_READ:
//                                         //TRACE(0x81);
//                                         do_sd_init_hw(0);
//                                         retval = sd_read_request_action( sd_queue[sd_queue_rx].host, sd_queue[sd_queue_rx].request );
//                                         break;
// 
//                                 case CMD_SD_WRITE:
//                                         //TRACE(0x82);
//                                         do_sd_init_hw(15);
//                                         retval = sd_write_request_action( sd_queue[sd_queue_rx].host, sd_queue[sd_queue_rx].request );
//                                         break;
// 
//                                 default:
//                                         retval = -1;
...
//                                 }
                                ...
                                //TRACE(0x83);
                                //printk(KERN_INFO "~~~~ %s - cmd=%d retval=%d\n", __FUNCTION__, sd_queue[sd_queue_rx].cmd, retval);
// sm                                 if (sdIrqData.error)
// sm                                         printk(KERN_ERR "++++ %s error=0x%02x cnt=%u pos=%u/%u\n", __FUNCTION__, sdIrqData.error, sdIrqData.errorCnt, sdIrqData.errorPos, sdIrqData.size);


// #if TEST_DRIVER
//                                 if (sd_queue[sd_queue_rx].cmd == CMD_SD_WELCOME) {
//                                         complete( sd_queue[sd_queue_rx].compl );
//                                 }
//                                 else {
//                                         complete( (struct completion*)(sd_queue[sd_queue_rx].queue) );
//                                 }
// #else
// sm                                 if (sd_queue[sd_queue_rx].request) {
                                        //TRACE(0x84);
// sm                                         spin_lock_irq( sd_queue[sd_queue_rx].queue->queue_lock);
// sm                                         blk_requeue_request(sd_queue[sd_queue_rx].queue, sd_queue[sd_queue_rx].request);
// sm ????                                    blk_end_request_all(sd_queue[sd_queue_rx].request, retval);
// sm                                         spin_unlock_irq( sd_queue[sd_queue_rx].queue->queue_lock);
                                        //TRACE(0x85);
// sm                                 }
// sm                                 else {
// sm                                         complete( sd_queue[sd_queue_rx].compl );
                                        //TRACE(0x86);
// sm                                 }
//#endif
// sm                                 BUF_INC(sd_queue_rx, MAX_SD_REQUESTS);
                         }
                         else {
                                sys_nanosleep(&sdmodem_poll_ktime, NULL);
                         }
...
// sm                         if (modem_lock_requested) {
// sm                                 if (!modem_lock_acknowleged) {
// sm                                        modemlock_callback(0/*dummy*/);
// sm                                       modem_lock_acknowleged = 1;
// sm                                 }
// sm                          }
...
// sm                                                modem_lock_acknowleged = 1;
                ...
                case sdmodem_state_modempausing:
                         ...
                         if (modem_lock_requested ||
                             ...
                             /*(!BUF_CAN_READ(sd_queue_wx, sd_queue_rx))*/ 
...
/* -------------------------------------------------------------------------- */
        ...
        * synchronous mode" in sh7727 datasheet */
        ...
        ctrl_outb(ctrl_inb(SCSSR) & ~(SCSSR_ORER | SCSSR_FER | SCSSR_PER), SCSSR);      /* clear any error flags */
...
{
	struct plat_sci_reg *reg = sci_getreg(p, offset);
        //printk(KERN_DEBUG"%s() %p 0x%lx %d %d\n",__FUNCTION__,p,(long unsigned int)p->mapbase,offset,value);
	if (reg->size == 8)
...
printk(KERN_DEBUG"%s():%d %d %d \n",__FUNCTION__,__LINE__,SCIx_NR_IRQS,irq);
...
printk(KERN_DEBUG"%s():%d\n",__FUNCTION__,__LINE__);
        ...
        //printk(KERN_INFO "queue_shutdown(sci ) requested.\n");
...
#warning "SH7727 does not have SCFCR register. (S.Minke)"
        printk(KERN_INFO"%s():%d: SCFC register not initialised\n",__FUNCTION__,__LINE__);
#else
...
/* Set up an universal multiplexed irq handler.
 *  For SCI/SD this handler is __never__ removed (also when devices are closed).
 */
static int sd_request_irq(void *ptr)
{
    printk(KERN_DEBUG"%s():%d\n",__FUNCTION__,__LINE__);    
    return sci_request_irq(&sd_pseudoport);
}
...
int shsci_init_work(struct list_head *spi_queue,void (*spi_sh_work)(struct list_head * spi_queue))
{
        ...
        printk(KERN_DEBUG"%s():%d %p\n",__FUNCTION__,__LINE__,sdmodem_share.spi_queue);
        return 0;
}
