...
static int spi_sh_setup(struct spi_device *spi)
{
	...
	udelay(10);
        ...
        ctrl_outb(0x00,SCSCR+BASE_SCS);
        ctrl_outb(SCSCMR_CA,SCSMR+BASE_SCS);
        ctrl_outb(0x20,SCBRR+BASE_SCS); 
        udelay(20);
        ctrl_outb( (SCSCR_RE|SCSCR_TE), SCSCR+BASE_SCS);
...
//#define TEST_SPI
        ...
        {/* test spi */ 
                ...
                for (i=0;i<10;i++)
                {
                    do {} while ((status = ctrl_inb( SCSSR + BASE_SCS ) & SCSSR_TDRE)!=SCSSR_TDRE);
                    ctrl_outb(i,  SCTDR + BASE_SCS);  
                    status &= ~SCSSR_TDRE;
                    ctrl_outb(status, SCSSR + BASE_SCS);
                }
        }  
#endif        
#endif
