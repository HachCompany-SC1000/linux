...
#include <../tty/serial/sh-sci-mux.h>
...
static irqreturn_t sd_receive_chars(int irq, void *ptr)
{
        /* Note: port == NULL */
        unsigned char c;
...
//#ifndef USE_MULTIPLEXER
static int sd_start_rxtx_irq(unsigned char *data, unsigned int len, unsigned char *rx_data,struct spi_message *mesg)
{
...
{
        return sd_receive_chars(irq,ptr);
        //return IRQ_HANDLED;
 }
			...
			struct spi_transfer *t)
{
 	...
 	unsigned char *tx_data,*rx_data;
        ...
        sd_start_rxtx_irq(tx_data, t->len,rx_data,mesg);
 	return retval;
}
...
#else
static void spi_sh_work(struct list_head * spi_queue)
{
        struct spi_sh_data *ss = container_of(spi_queue, struct spi_sh_data, queue);
#endif
	...
	while (!list_empty(&ss->queue)) {
		...
		list_for_each_entry(t, &mesg->transfers, transfer_list) { /* eventuell in einzelne transaktionen aufteilen */
 			pr_debug("tx_buf = %p, rx_buf = %p\n",
 					t->tx_buf, t->rx_buf);
 			pr_debug("len = %d, delay_usecs = %d\n",
 					t->len, t->delay_usecs);
                        if (t->tx_buf) {
				ret = spi_sh_send(ss, mesg, t);/* send blockiert */
#ifdef DEBUG_MMC
{
    int i=0;
    unsigned char buffer[64];
    while(1){
        snprintf(buffer,64,"tx %02x %02x %02x %02x %02x %02x %02x %02x "\
                           "%02x %02x %02x %02x %02x %02x %02x %02x", \
                        *((unsigned char *)(t->tx_buf)+(i)), 
                        *((unsigned char *)(t->tx_buf)+(i+1)), 
                        *((unsigned char *)(t->tx_buf)+(i+2)), 
                        *((unsigned char *)(t->tx_buf)+(i+3)), 
                        *((unsigned char *)(t->tx_buf)+(i+4)), 
                        *((unsigned char *)(t->tx_buf)+(i+5)), 
                        *((unsigned char *)(t->tx_buf)+(i+6)), 
                        *((unsigned char *)(t->tx_buf)+(i+7)), 
                        *((unsigned char *)(t->tx_buf)+(i+8)), 
                        *((unsigned char *)(t->tx_buf)+(i+9)), 
                        *((unsigned char *)(t->tx_buf)+(i+10)), 
                        *((unsigned char *)(t->tx_buf)+(i+11)), 
                        *((unsigned char *)(t->tx_buf)+(i+12)), 
                        *((unsigned char *)(t->tx_buf)+(i+13)), 
                        *((unsigned char *)(t->tx_buf)+(i+14)), 
                        *((unsigned char *)(t->tx_buf)+(i+15))); 
        i+=16;
    ...
    if (t->rx_buf){
        i=0;
        while(1){
            snprintf(buffer,64,"rx %02x %02x %02x %02x %02x %02x %02x %02x "\
                            "%02x %02x %02x %02x %02x %02x %02x %02x", \
                            *((unsigned char *)(t->rx_buf)+(i)), 
                            *((unsigned char *)(t->rx_buf)+(i+1)), 
                            *((unsigned char *)(t->rx_buf)+(i+2)), 
                            *((unsigned char *)(t->rx_buf)+(i+3)), 
                            *((unsigned char *)(t->rx_buf)+(i+4)), 
                            *((unsigned char *)(t->rx_buf)+(i+5)), 
                            *((unsigned char *)(t->rx_buf)+(i+6)), 
                            *((unsigned char *)(t->rx_buf)+(i+7)), 
                            *((unsigned char *)(t->rx_buf)+(i+8)), 
                            *((unsigned char *)(t->rx_buf)+(i+9)), 
                            *((unsigned char *)(t->rx_buf)+(i+10)), 
                            *((unsigned char *)(t->rx_buf)+(i+11)), 
                            *((unsigned char *)(t->rx_buf)+(i+12)), 
                            *((unsigned char *)(t->rx_buf)+(i+13)), 
                            *((unsigned char *)(t->rx_buf)+(i+14)), 
                            *((unsigned char *)(t->rx_buf)+(i+15))); 
            i+=16;
            printk(KERN_DEBUG"%s",buffer);
            if (i>=t->len) break;
        }
...
static int spi_sh_setup(struct spi_device *spi)
{
	...
	struct spi_sh_data *ss = spi_master_get_devdata(spi->master);
        ...
        irq = ss->irq;
        ctrl_outw( (ctrl_inw(SCPCR) & (~0x000F)),SCPCR);
...
#else
        printk(KERN_DEBUG"%s():%d %p\n",__FUNCTION__,__LINE__,&ss->queue);
        shsci_init_work(&ss->queue, spi_sh_work);
#endif
...
#else            
            ret = shsci_request_irq(irq+i, sci_irq_desc[i].handler);
#endif            
            if (ret < 0) {
