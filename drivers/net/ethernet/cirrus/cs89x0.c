...
#elif defined(CONFIG_SH_SC1000)
static unsigned int netcard_portlist[] __initdata =
   { 0xb4000003	, 0};
static unsigned int cs8900_irq_map[] =
   { /*IRQ2_IRQ */ 34,0,0,0};
extern uint32_t sc1000_mac[];

#define IRR0 *(volatile unsigned char *)0xA4000004
#define IRQ2R ((unsigned char)(1<<2))
...
readword(unsigned long base_addr, int portno)
{
	return ctrl_inw(base_addr + portno);
}

static void
writeword(unsigned long base_addr, int portno, u16 value)
{
	ctrl_outw(value, base_addr + portno);
}
#endif
...
cs89x0_probe1(struct net_device *dev, int ioaddr, int modular)
{
	...
	reset_chip(dev);
	if (IRR0 & IRQ2R) // see comment below (jsp)
		IRR0 = ~IRQ2R;
...
#elif defined(CONFIG_SH_SC1000)
	if(1){
	    unsigned int sc1000_mac_isnotzero = 0;
	    /* Read the MAC address from a variable which is initialised in
	     * the board setup routines */
	    for(i=0;i<ETH_ALEN;i++){
		dev->dev_addr[i] = sc1000_mac[i];
		sc1000_mac_isnotzero |= sc1000_mac[i];
	    }
	    if (!sc1000_mac_isnotzero) {  // set a nonzero mac (ETH_ALEN=6):
		    dev->dev_addr[0] = 0x00;
		    dev->dev_addr[1] = 0x0E;
		    dev->dev_addr[2] = 0x1C;
		    dev->dev_addr[3] = 0x02;
...
#if defined(CONFIG_SH_HICOSH4) || defined(CONFIG_SH_SC1000) /* no EEPROM , don't hazzle with it here */
	if (1)
...
release_irq:
	...
	case A_CNF_MEDIA_10B_T:
		...
		// IMM_BIT is not running, overwritten! (Rainer)
		result = DETECTED_RJ45H; //Enable eth without cable connection!!!
		break;
...
static irqreturn_t net_interrupt(int irq, void *dev_id)
{
	...
	if (net_debug > 5) printk("%s: net_interrupt\n", dev->name);

#if defined(CONFIG_SH_SC1000)
	/* CS8900 interrupt on SC1000 is edge sensitive and needs to be
	 acknowledged */

	/*from the SH7727 Hardware Manual (jsp):
...
#define READ_TYPE_BYTES (2*ETH_ALEN+2)
	unsigned char sk_buff_data[READ_TYPE_BYTES];
	unsigned short type;
	...
	/* read up to type */
	if (length < READ_TYPE_BYTES) {
		printk("net_rx() - found length=%d\n", length);
		return;
	}
	readwords(ioaddr, RX_FRAME_PORT, sk_buff_data, READ_TYPE_BYTES/2);

	/* filter by type */
	type = (sk_buff_data[ETH_ALEN+ETH_ALEN] << 8) | sk_buff_data[ETH_ALEN+ETH_ALEN+1];
	if ( !(type<=0x05DC || type==0x0800 || type==0x0806) ) {
		/* drop this packet */
		drop_cnt++;
		dev->stats.rx_dropped++; /* needed here ??? (jsp) */
		return;
	}
	...
	/* read rest of data */
	memcpy(skb->data, sk_buff_data, READ_TYPE_BYTES);
	readwords(ioaddr, RX_FRAME_PORT, skb_put(skb, length)+READ_TYPE_BYTES, (length >> 1)-(READ_TYPE_BYTES/2));
	if (length & 1)
